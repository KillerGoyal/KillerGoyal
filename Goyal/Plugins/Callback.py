...
from Goyal import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from Goyal.Core.PyTgCalls import Goyal, Queues
from Goyal.Core.PyTgCalls.Converter import convert
from Goyal.Core.PyTgCalls.Downloader import download
from Goyal.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from Goyal.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Goyal.Decorators.admins import AdminRightsCheckCB
from Goyal.Decorators.checker import checkerCB
from Goyal.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from Goyal.Utilities.changers import time_to_seconds
from Goyal.Utilities.chat import specialfont_to_normal
from Goyal.Utilities.paste import isPreviewUp, paste_queue
from Goyal.Utilities.theme import check_theme
from Goyal.Utilities.thumbnails import gen_thumb
from Goyal.Utilities.timer import start_timer
from Goyal.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await Goyal.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await Goyal.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await Goyal.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await Goyal.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await Goyal.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
