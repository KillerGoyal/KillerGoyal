...
from Goyal import BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Goyal.Database import (add_gban_user, add_off, add_on, add_sudo,
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m Goyal")
        else:
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.system(f"kill -9 {os.getpid()} && python3 -m Goyal")
...
async def ban_globally(_, message):
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            ...
            await message.reply_text("You want to block a sudo user? KIDDO!!")
        else:
